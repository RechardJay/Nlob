class MarkdownLoader {
    ...
    async renderBlogContent(containerId) {
        ...
        try {
            ...
            if (!blog) {
                container.innerHTML = this.getNotFoundError(`请求的文章 "${postId}" 不存在`);
                return;
            }
            ...
            container.innerHTML = this.getErrorContent('文章加载失败，请刷新页面重试。');
        }
    }
        ...
        return `
            <article class="blog-post">
                <header class="blog-header">
                    ...
                    <div class="blog-meta">
                        <span class="publish-date">发布于 ${blogInfo.date || metadata.date || '未知日期'}</span>
                        ${blogInfo.tags ? `<span class="blog-tags"> | 标签: ${blogInfo.tags}</span>` : ''}
                    </div>
                </header>
    ...
    async loadBlogContent(filename) {
        try {
            const response = await fetch(`${this.blogsPath}${filename}`);
            if (!response.ok) {
        ...
        try {
            ...
            const html = blogs.map(blog => `
                <article class="blog-card">
                    ...
                    <div class="blog-excerpt">${this.escapeHtml(blog.excerpt)}</div>
                </article>
            `).join('');
